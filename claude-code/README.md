...
Auto-injection behavior (when to inject, minimum prompt length) is configured on the server (`INJECT_TIMING`, `MIN_QUERY_CHARS`); all clients apply it when a session starts.
