"""Live scoping tests against the Mem0 Platform: run with NEATMEM_API_KEY set, skipped otherwise."""
...
pytestmark = pytest.mark.skipif(
    not os.environ.get("NEATMEM_API_KEY"), reason="NEATMEM_API_KEY is required for live scoping tests"
)
...
class Namespace:
    ...
    def __init__(self, tmp: Path):
        ...
        os.environ["NEATMEM_CODE_DATA_DIR"] = str(tmp / "data")
        os.environ["NEATMEM_CODE_TELEMETRY"] = "false"
        os.environ["NEATMEM_CODE_EXTRACTION_WAIT_SECONDS"] = "180"
        self.store = memory_core.EvidenceStore()
        self.reads = 0

    def as_user(self, name: str) -> str:
        os.environ["NEATMEM_CODE_USER_ID"] = self.users[name]
