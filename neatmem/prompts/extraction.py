"""
记忆提取 prompt — 基于 mem0 ADDITIVE_EXTRACTION_PROMPT 照搬，增加 Same-Batch Context Resolution 规则。

原始来源: mem0/configs/prompts.py
修改记录:
  - 2026-05-17: 在 Integrity Rules 的 "No Detail Contamination" 规则后追加
    "Same-Batch Context Resolution" 规则。

    问题：原始 prompt 的 "No Detail Contamination" 规则禁止从其他记忆导入细节，
    但过度泛化导致同批次内的新消息之间也不补全上下文。

    Bad case：用户连发两条消息——
      1) "我最近用 mem0 开发记忆模块"
      2) "在开发中遇到了记忆不完整问题"
    原始 prompt 抽出："用户在开发中遇到了记忆不完整问题"
    修改后抽出："用户在开发 mem0 记忆模块时遇到了记忆不完整问题"

    规则要点：从 Existing/Recent Memories 导入细节仍然禁止（防止幻觉），
    但同一批 New Messages 内部，后续消息依赖前述消息时，必须补全上下文，
    且必须保留具体专有名词，不能用模糊指代（如"开发中"）替代（如"开发 mem0 记忆模块时"）。

    详见 docs/20260517-openclaw-auto-capture-context-test.md
"""
...
# ---------------------------------------------------------------------------
# System Prompt — 照搬 mem0 ADDITIVE_EXTRACTION_PROMPT，仅追加一条规则
# ---------------------------------------------------------------------------
...
- **No Meta-Extraction**: Extract the CONTENT of what was shared, not a description of the user's action. When a user shares a document, data, or reference material, extract the actual facts FROM that material.
  ...
  [规则 A] 禁止从已有记忆污染细节：从 New Messages 提取时，禁止导入或合并来自 Existing Memories 或 Recent Memories 的细节，除非新消息显式引用了这些细节。
- **No Detail Contamination from Context**: When extracting from New Messages, do NOT import or merge details from Existing Memories or Recent Memories into the new extraction UNLESS the new message explicitly references those details. If the New Message says "I had a great meal" and an Existing Memory says "User's favorite restaurant is Olive Garden," do NOT produce "User had a great meal at Olive Garden" — the new message never mentioned the restaurant. Each extraction must be faithful to its source message only.
  [规则 B] 同批次上下文补全（对规则 A 的例外）：规则 A 禁止从已有记忆导入细节——这是正确的，防止幻觉。但它会误伤同批次内新消息之间的上下文依赖。当同一批 New Messages 中，后面的消息通过代词、省略或上下文引用依赖前面的消息时，必须将前面消息中的具体细节（专有名词、实体名等）补入提取结果，使每条记忆自包含。且禁止用模糊指代替代具体细节。
...
# ---------------------------------------------------------------------------
# Prompt Builder — 照搬 mem0 generate_additive_extraction_prompt 及辅助函数
# ---------------------------------------------------------------------------
