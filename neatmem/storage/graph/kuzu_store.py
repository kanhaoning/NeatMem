"""Kuzu graph store — faithful port of mem0 1.0.11 MemoryGraph (kuzu_memory.py).

Per plan section 4.5 纪律原则: the graph build/recall LOGIC is copied verbatim
from mem0/memory/kuzu_memory.py. Only the following are changed (wiring, not
algorithm):
  - __init__ takes explicit params instead of mem0's Config object
  - self.llm.generate_response(...) -> self.adapter.llm_generate(...)
  - self.embedding_model.embed(...) -> self.adapter.embed(...)
  - self.config.graph_store.* -> instance attributes set from env/explcit args

Schema, Cypher queries, embedding-similarity node merge (threshold 0.7),
soft-delete of contradictory relations, BM25 rerank — all identical to mem0.
Algorithm divergence breaks the section-8 equivalence test (Jaccard >= 0.8).
"""
...
from neatmem.signals.graph.prompts import (
    EXTRACT_RELATIONS_PROMPT,
    format_entities,
    get_delete_messages,
    remove_spaces_from_entities,
)
from neatmem.signals.graph.tools import (
...
class KuzuGraphStore:
    """File-based graph store backed by Kuzu, algorithm-identical to mem0 1.0.11.

    Args:
        db_path: Path to the Kuzu database directory.
        adapter: GraphAdapter providing llm_generate() and embed().
        threshold: Embedding cosine similarity threshold for node matching (default 0.7).
        custom_prompt: Optional extra instruction injected into EXTRACT_RELATIONS_PROMPT.
        llm_provider: "openai" or "openai_structured" (selects struct vs non-struct tools).
    """
    ...
    def __init__(
        ...
        self.embedding_dims = adapter.embedding_dims
        ...
        # Thread safety: kuzu.Connection may not be thread-safe (like DuckDB).
        # When the FastAPI server calls gs.search() via asyncio.to_thread,
        # multiple threads can hit kuzu_execute concurrently. Lock only the
        # kuzu part (fast, ~0.1s/query); LLM/embedding calls stay unlocked.
        # Must be set BEFORE kuzu_create_schema() which calls kuzu_execute.
    ...
    # ------------------------------------------------------------------
    # Schema & execution — verbatim from mem0
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Public API — verbatim orchestration from mem0
    # ------------------------------------------------------------------
    def add(self, data, filters):
        """Add data to the graph. Mirrors mem0 MemoryGraph.add exactly."""
    ...
    def search(self, query, filters, limit=5):
        """Search for related graph data. Mirrors mem0 MemoryGraph.search exactly."""
    ...
    def delete(self, data, filters):
        """Delete graph entities. Mirrors mem0 MemoryGraph.delete exactly."""
        try:
            entity_type_map = self._retrieve_nodes_from_data(data, filters)
    ...
    # ------------------------------------------------------------------
    # Extraction — verbatim from mem0 (LLM calls routed through adapter)
    # ------------------------------------------------------------------
    def _retrieve_nodes_from_data(self, data, filters):
        """Extracts all the entities mentioned in the query. Verbatim mem0 logic."""
        _tools = [EXTRACT_ENTITIES_TOOL]
        if self.llm_provider in ["azure_openai_structured", "openai_structured"]:
            _tools = [EXTRACT_ENTITIES_STRUCT_TOOL]
        search_results = self.adapter.llm_generate(
            messages=[
                {
                    "role": "system",
    ...
    def _establish_nodes_relations_from_data(self, data, filters, entity_type_map):
        """Establish relations among the extracted nodes. Verbatim mem0 logic."""
        ...
        if self.custom_prompt:
            ...
            system_content = system_content.replace("CUSTOM_PROMPT", f"4. {self.custom_prompt}")
            messages = [
        ...
        extracted_entities = self.adapter.llm_generate(
            messages=messages,
            tools=_tools,
        )
    ...
    def _search_graph_db(self, node_list, filters, limit=100, threshold=None):
        """Search similar nodes and their relations. Verbatim mem0 logic."""
        result_relations = []
        ...
        for node in node_list:
            n_embedding = self.adapter.embed(node)
    ...
    def _get_delete_entities_from_search_output(self, search_output, data, filters):
        """Get entities to delete. Verbatim mem0 logic (the soft-delete step Kimi skipped)."""
        ...
        memory_updates = self.adapter.llm_generate(
            messages=[
    ...
    def _delete_entities(self, to_be_deleted, filters):
        """Delete entities from the graph. Verbatim mem0 Cypher."""
        user_id = filters["user_id"]
    ...
    def _add_entities(self, to_be_added, filters, entity_type_map):
        """Add new entities to the graph. Verbatim mem0 Cypher (4 branches)."""
        user_id = filters["user_id"]
        agent_id = filters.get("agent_id", None)
        run_id = filters.get("run_id", None)
        results = []
        for item in to_be_added:
            ...
            # embeddings
            source_embedding = self.adapter.embed(source)
            dest_embedding = self.adapter.embed(destination)
