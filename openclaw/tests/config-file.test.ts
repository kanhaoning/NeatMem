...
describe("writePluginAuth", () => {
  it("creates nested structure from scratch when no config exists", () => {
    ...
    expect(written.plugins.entries["openclaw-neatmem"].enabled).toBe(true);
    expect(written.plugins.entries["openclaw-neatmem"].config.apiKey).toBe(
      "sk-new",
    );
    expect(written.plugins.entries["openclaw-neatmem"].config.userId).toBe("u1");
  });
  ...
  it("skips undefined values", () => {
    ...
    const written = JSON.parse(mockWriteText.mock.calls[0][1]);
    const cfg = written.plugins.entries["openclaw-neatmem"].config;
