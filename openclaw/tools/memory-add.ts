...
export function createMemoryAddTool(deps: ToolDeps) {
  ...
  return {
    name: "memory_add",
    label: "Memory Add",
    description: "Save important information in long-term memory via NeatMem. Use for preferences, facts, decisions, and anything worth remembering.",
    ...
    async execute(_toolCallId: string, params: Record<string, unknown>) {
      ...
      try {
        ...
        if (skillsActive) {
          ...
          api.logger.info(`openclaw-neatmem: stored ${count} memor${count === 1 ? "y" : "ies"} (infer=false, category=${category ?? "none"})`);
        ...
        // NOTE: 前端去重 search 已移除 — 后端 add_memories() 会做完整的向量召回+LLM三分类去重，
        // 这里再做一次纯阈值 search 既无返回值判断也无去重决策，属于无效网络请求。
